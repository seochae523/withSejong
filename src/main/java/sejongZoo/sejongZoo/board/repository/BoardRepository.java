...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query("SELECT b from board b left join fetch b.user u left join fetch b.image i left join fetch b.tag t where b.deleted = false order by b.createdAt DESC ")
    ...
    @Query("select b from board b join fetch b.user u left join fetch b.image i left join fetch b.tag t " +
            ...
            "or b.id=(select t2.board.id from tag t2 where t2.category like concat(:keyword, '%'))) and b.deleted=false " +
            "order by b.createdAt desc")
    Page<Board> searchWithKeyword(@Param("keyword") String keyword, Pageable pageable);

    @Query("select b from board b join fetch b.user u left join fetch b.image i left join fetch b.tag t" +
            " where u.studentId=:studentId and b.deleted=false")
    Page<Board> findByStudentId(@Param("studentId") String studentId, Pageable pageable);


    @Query("select b from board b left join tag t where t.category in :tags and b.deleted=false")
